...
def get_playlist():
    ...
    try:
        ...
        search_results = conn.search_tracks(
            seed_genres=genres, seed_artists=artists, target_bpm=int(bpm_range["optimal"]), limit=50
        )
        ...
        if not tracks:
            ...
            diag_results = conn.search_tracks(
                seed_genres=genres, seed_artists=artists, target_bpm=None, limit=5
            )
...
def save_playlist():
    ...
    print(
        f"Attempting to save playlist '{name}' with {len(track_uris) if track_uris else 0} tracks"
    )
