...
class SpotifyConnector:
    ...
    def search_tracks(self, seed_genres: list[str], seed_artists: list[str] = None, target_bpm: int = None, limit: int = 20) -> list[dict]:
                    ...
                    for track in tracks_data.get("items", []):
                        if track and track.get("id") and track["id"] not in all_tracks:
                            all_tracks[track["id"]] = {
                                "id": track["id"],
                                "name": track["name"],
                                "artists": [a["name"] for a in track["artists"]] if track.get("artists") else [],
                                "duration_ms": track.get("duration_ms", 0),
                                "uri": track.get("uri"),
                                "popularity": track.get("popularity", 50),
                                "source": "search"
                            }
            if len(all_tracks) >= 150: break
        ...
        if target_bpm:
            ...
            for tempo in search_tempos:
                ...
                p_params = {"q": bpm_query, "type": "playlist", "limit": 2}
            p_resp = requests.get("https://api.spotify.com/v1/search", headers=headers, params=p_params)
